@if(count($activities) > 0)
    ...
    <div id="act-{{$row->id}}" class="act odd">
        ...
        <div class="col-7 col-md-4 fl">{{$row->activity_name}}</div>
        <div class="col-8 col-md-6 fl">{{substr($row->activity_desc, 0, 250).$ext}}</div>
        <div class="col-2 col-md-1 fl"><div class="row">
            ...
            <div class="fr link" style="position: absolute; right: 0;"><i id="{{$row->id}}" title="View Media" class="edit-media fa fa-image"></i></div>
        </div></div>
        <div class="clr"></div>
    </div>
    @endforeach
