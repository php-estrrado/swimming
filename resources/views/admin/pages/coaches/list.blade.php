...
<!--                <button id="delete-all-btn" type="button" class="btn btn-sm btn-danger mt-1 mb-1 float-right"><i class="fa fa-trash"></i> Delete Selected</button>-->
                <a href="{{url('/admin/user/coach/0')}}" class="btn-padd btn btn-sm btn-primary float-right"><i class="fa fa-plus"></i> Add Coach</a>
            </div>
            <div id="coach_list" class="card-body table-card-body">
                @include('admin.pages.coaches.list.content')
            </div>
...
$(document).ready(function () {
    $("body").on("change", ".status-btn", function () {
        ...
        var url         =   '{{url("/admin/coach/update/status")}}';;
    ...
    $("body").on("click", ".coachDelBtn", function () { 
        ...
        updateStatus(id,cId,status,url,'coach_list','delete',smsg,'users');
    });
