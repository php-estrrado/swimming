...
class Chat extends Model
{
    ...
    static function chatList($userId,$srField='',$srVal='',$type=''){
        if($srField             !=  ''  && $srVal != ''){
        ...
        if($query->count()      >   0){ foreach($query->get() as $row){ 
            if($row->group      ==  0){   
                ...
                if($member->avthar  != '' && $member->avthar != NULL){ $avthar  = url('/storage'.$member->avthar); }else{ $avthar = url('storage/app/public/user.png'); }
            }else{ 
                $name           =   $row->name;  
                if($row->group_img  != '' && $row->group_img != NULL){ $avthar  = url('/storage'.$row->group_img); }else{ $avthar = url('storage/app/public/group.png'); }
                $chatMembers    =   Chat::getGroupMembers($userId,$row->created_by,$row->id,'');
            }
            ...
            if($srField         !=  ''  && $srVal != ''){
               ...
               if($qry->count() >   0){ foreach($qry->get() as $msg){
    ...
    static function createNewChat($userId,$coachId){
        $query                  =   DB::table('chat as C')->select('C.id')->leftJoin('chat_members as CM', 'C.id','=','CM.chat_id')
                                    ->where(function ($qry) use ($userId,$coachId){ $qry->where(function ($query) use ($userId,$coachId){ $query->where('CM.user_id',$userId)->where('C.created_by',$coachId); })
                                    ->orWhere(function ($query) use ($userId,$coachId){ $query->where('CM.user_id',$coachId)->where('C.created_by',$userId); }); })->where('C.status',1);
        if($query->count()      ==  0){ 
            $chatId             =   DB::table('chat')->insertGetId(['created_by'=>$userId,'created_on'=>date('Y-m-d H:i:s'),'member'=>$coachId]);
            if($chatId){ DB::table('chat_members')->insert(['chat_id'=>$chatId,'user_id'=>$coachId]); }
        ...
        return Chat::getChatHistory($userId,$chatId);
    }
    
    static function getChatHistory($userId,$chatId){
        $chat                   =   DB::table('chat')->where('id',$chatId)->where('status',1)->first();
        $updReadStatus          =   Chat::updateReadStatus($userId,$chat);
        DB::table('users')->where('id',$userId)->update(['chat_notify'=>0]);
        ...
        if($chat->group == 0){    
            ...
            $name               =   $member->name;  $chatMembers = null;
        }else{ 
            $name               =   $chat->name;  
            $chatMembers        =   Chat::getGroupMembers($userId,$chat->created_by,$chat->id,'');
        }
        $res['chat_id']         =   $chat->id;
        $res['name']            =   $name;
    //    $res['group']           =   $chat->group;
        $res['avthar']          =   Chat::getChatAvthar($userId,$chat->id,$chat->created_by)[0];
    //    $res['mambers']         =   $chatMembers;
        $messages               =   Chat::getChatMessages($chatId,$userId);
    ...
    static function getChatAvthar($userId,$chatId,$creator){
    ...
    static function getChatMessages($id,$userId){
        $data                       =   array(); $read = '';
        $query                      =   DB::table('chat_messages')->where('chat_id',$id)->where('deleted',0)->where('status',1)->orderBy('id','asc');
        if($query->count()          >   0){ 
    ...
    static function sendChatMessage($userId, $post){
        ...
        $friends                =   Chat::getGroupMembers($userId,$chat->created_by,$post->chat_id,'all');
    ...
    static function searchChat($user,$childId, $post){
        ...
        $data['coach_list']     =   Chat::getChatCoaches($user,$childId,'D.name',$post->search,'data');
        $data['chat_list']      =   Chat::chatList($user->id,'message',$post->search,'data');
    ...
    static function sendPush($user){
        ...
        Functions::pushNotify_test('Merlin','Test Message',$user->deviceToken,1,'chat',$notifyParms,$user->os);
        if($user->os == 'ios'){
    ...
    // For webhook for dot net team
    static function saveResponse($post){
        $name = 'Merlin'; $email = 'merlin@estrrado.com';
        $data   =   ['name'=>$post->name,'phone'=>$post->phone,'email'=>$post->email,'nationality'=>$post->nationality,'qualification'=>$post->qualification,'modes'=>$post->modes,'faculties'=>$post->faculties,'courses'=>$post->courses];
